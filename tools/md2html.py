...
class Markdown2Html:
  ...
  def setStyle(self, cssfile=None):
    ...
    if cssfile is None:
      ...
      with open(cssfile, 'r') as f:
        css = f.read()
        self.headTag = self.headTag[:-7] + f'{css}' + self.headTag[-7:]
  ...
  def convert(self, infile, outfile=None, prettify=False):
    ...
    rawhtml = self.headTag + markdown2.markdown(markdownText)
...
if __name__ == '__main__':
  m2h = Markdown2Html('../web/learning_style.css')
  m2h.convert('../res/learn.md', '../res/learn.html')          